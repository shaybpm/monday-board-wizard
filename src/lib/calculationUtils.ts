...
export const processBoardData = async (
  ...
  try {
    ...
    if (useSpecificFormula) {
      // Handle the special case for Hebrew columns like in testCalculation
      let validItems = 0;
      let successCount = 0;
      ...
      let summaryMessage = `Processed ${items.length} items with Hebrew columns calculation:\n`;
      summaryMessage += `✅ ${successCount} calculations successful\n`;
      summaryMessage += `❌ ${items.length - successCount} calculations failed\n`;
